...
alfa = 2*pi/180; % Angolo di incidenza in radianti

%% LOAD POINTS: Panel extrema
if body==0 % CIRCULAR CYLINDER:
    Np = 100; R=1; Xc = 0; Yc = 0; % numero pannelli, raggio, x centro, y centro
    Xlo = -0.3; Ylo=-0.3;
    Xup = 0.3; Yup = 0.3;
...
elseif body==2 % AIRFOIL
    ...
    Np = size(xe,1)-1; % numero pannelli
end
...
% Kutta condition
for j = 1:Np
    [sou.u,sou.v] = source(xe(j),ye(j),xe(j+1),ye(j+1),xp(),yp()); 
    [vor.u,vor.v] = vortex(xe(j),ye(j),xe(j+1),ye(j+1),xp(),yp());
    A(Np+1,j) = ;
    A(Np+1,Np+1) = ;
    [sou.u,sou.v] = source(xe(j),ye(j),xe(j+1),ye(j+1),xp(),yp());    
    [vor.u,vor.v] = vortex(xe(j),ye(j),xe(j+1),ye(j+1),xp(),yp()); 
...
sigma = ; % Soluzione intensità sorgenti
gamma= ; % Soluzione intensità vortice
...
for i =1:Np
    ...
    for j = 1:Np
        [sou.u,sou.v] = source(xe(j),ye(j),xe(j+1),ye(j+1),xp(i),yp(i));
        [vor.u,vor.v] = vortex(xe(j),ye(j),xe(j+1),ye(j+1),xp(i),yp(i));
       velu(i) = velu(i) + *sou.u + *vor.u;
       velv(i) = velv(i) + *sou.v + *vor.v;
    end
end
