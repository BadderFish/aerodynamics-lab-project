...
U = 1; % Velocità asintotica
alfa = 1; % Angolo di incidenza in radianti

%% LOAD POINTS: Panel extrema
if body==0 % CIRCULAR CYLINDER:
    Np = 101; R=1; Xc = 0; Yc = 0; % numero pannelli, raggio, x centro, y centro
    Xlo = -0.3; Ylo=-0.3;
    Xup = 0.3; Yup = 0.3;
...
elseif body==1     % RECTANGULAR CYLINDER
    Npx=40; Npy=8; Np=2*(Npx+Npy); % numero pannelli in x e y
    D=1; L=5*D; Xc=0; Yc=0;
...
elseif body==2 % AIRFOIL
    ...
    [xd, yd, xv, yv] = NACA_4d (M, P, SS, n, 'halfcos');
...
% Punti di Controllo dei pannelli come punto medio tra gli estremi del
% pannello
xp = 0.5*(xe(1:Np) + xe(2:Np+1));
yp = 0.5*(ye(1:Np) + ye(2:Np+1));
% Normali al pannello
normal = zeros(Np,2); % Componente x nella colonna 1, componente y nella colonna 2
normal(:,1) = -(ye(2:Np+1)-ye(1:Np))./(sqrt((ye(2:Np+1)-ye(1:Np)).^2+(xe(2:Np+1)-xe(1:Np)).^2));
normal(:,2)=   (xe(2:Np+1)-xe(1:Np))./(sqrt((ye(2:Np+1)-ye(1:Np)).^2+(xe(2:Np+1)-xe(1:Np)).^2)); 
% Tangenti al pannello 
tangent = zeros(Np,2); % Componente x nella colonna 1, componente y nella colonna 2
tangent(:,1) = (xe(2:Np+1)-xe(1:Np))./(sqrt((ye(2:Np+1)-ye(1:Np)).^2+(xe(2:Np+1)-xe(1:Np)).^2));
tangent(:,2) = (ye(2:Np+1)-ye(1:Np))./(sqrt((ye(2:Np+1)-ye(1:Np)).^2+(xe(2:Np+1)-xe(1:Np)).^2));
% Lunghezze dei pannelli
...
% Kutta condition
b(Np+1) = -Uinf(1)*tangent(1,1) - Uinf(2)*tangent(1,2) - Uinf(1)*tangent(Np,1) - Uinf(2)*tangent(Np,2);

%% Solve the linear system
sol = linsolve(A,b);

sigma = sol(1:Np); % Soluzione intensità sorgenti
gamma= sol(Np+1); % Soluzione intensità vortice
...
% Computation of Cp and Cl
% from Kutta joukowski
Gamma=gamma*Np*mean(length);
Lkj=1.225*U*Gamma;
CLkj=2*Lkj/1.225/U^2/1 

% from definition
Vt = velu.*tangent(:,1) + velv.*tangent(:,2);
Cp = 1-Vt.^2/(Uinf(1)^2+Uinf(2)^2);
...
if plot_fig==1
    ...
    % Plot geometria, control point e pannelli
    ...
    Nfx=100; Nfy = 100;
    x=linspace(-0.5,2,Nfx);
    y=linspace(-0.6,0.6,Nfy);
    ...
    contourf(X',Y',Ufield-Uinf(1),100,'LineStyle','None'); % Plot of x component of velocity field
    title('U Velocity field')
    colormap(bluewhitered(256)); %flipud(hot)
    ...
    contourf(X',Y',abs(Vfield-Uinf(2)),100,'LineStyle','None'); % Plot of y component of velocity field, abssolute values, perturbation, global
    ...
    if body==2
        ...
        plot(xp,-Cp,'LineWidth',1.5,'Color','r') % Plot Cp
        tx=xlabel('$x$');
        ty=ylabel('$-Cp$');
        tx.Interpreter='latex';
        ty.Interpreter='latex';
...
[X,Y]=meshgrid(x,y);
figure(6); hold on; box on; % Vector plot of velocity field
